...
type FormPageBuilderWaitlistProps = {
  isOpen: boolean;
  onClose: () => void;
};

const FormPageBuilderWaitlist = ({
  isOpen,
  onClose
}: FormPageBuilderWaitlistProps) => {
  ...
  useEffect(() => {
    if (isOpen && !firstOpened) {
      segmentFormEvent("Form Viewed", {
        formName: "page builder waitlist"
      });
      setFirstOpened(true);
    }
...
export default FormPageBuilderWaitlist;
